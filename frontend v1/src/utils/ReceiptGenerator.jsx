...
const ReceiptDocument = ({ order }) => {
  ...
  const customerMobile = order.address.phone || '9999999999';
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.productTable}>
          ...
          {order.items?.map((item, index) => (
            <View key={index} style={styles.tableRow}>
              <Text style={styles.col1}>{item.product_name || item.product_variant?.product?.name || 'N/A'}</Text>
              <Text style={styles.col2}>{item.product_variant?.size || 'Free Size'}</Text>
              <Text style={styles.col3}>{item.quantity || 1}</Text>
              <Text style={styles.col4}>NA</Text>
              <Text style={styles.col5}>{order.order_number || order.id} {item.id}</Text>
            </View>
          ))}
        </View>
        ...
        <View style={styles.invoiceHeader}>
          <Text>BILL OF SUPPLY/COMMERCIAL INVOICE - Original For Recipient</Text>
        </View>
        ...
        <View style={styles.invoiceBody}>
          ...
          <View style={styles.soldBy}>
            ...
            <Text style={[styles.addressText, { fontWeight: 'bold', marginTop: 8 }]}>
            ...
            <View style={styles.invoiceMeta}>
             <View style={styles.metaItem}>
                ...
                <Text>{new Date(order.created_at).toLocaleDateString('en-GB')}</Text>
              </View>
              <View style={styles.metaItem}>
          ...
          {order.items?.map((item, index) => (
            <View key={index} style={styles.itemsTableRow}>
              <Text style={styles.itemCol1}>
                {item.product_name || item.product_variant?.product?.name || 'N/A'} - {item.product_variant?.size || 'Free Size'}
              </Text>
              <Text style={styles.itemCol2}>{item.quantity || 1}</Text>
              <Text style={styles.itemCol3}>Rs . {parseFloat(item.price || 0).toFixed(2)}</Text>
              <Text style={styles.itemCol4}></Text>
        ...
        <View style={styles.footerNote}>
          <Text>
            This is a computer generated invoice and does not require signature. Other charges are charges 
            that are applicable to your order and include charges for logistics fee (where applicable). 
